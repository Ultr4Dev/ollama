...
func TestParseNameParts(t *testing.T) {
	cases := []struct {
		...
		{
			in: "nexus.shitcloud.se/repository/ollama-COMBINED/library/dolphin-mistral:7b-v2.6-dpo-laser-q6_K",
			want: Name{
				Host:      "nexus.shitcloud.se/repository/ollama-COMBINED",
				Namespace: "library",
				Model:     "dolphin-mistral",
				Tag:       "7b-v2.6-dpo-laser-q6_K",
			},
			wantFilepath: filepath.Join("nexus.shitcloud.se/repository/ollama-COMBINED", "library", "dolphin-mistral", "7b-v2.6-dpo-laser-q6_K"),
		},
		{
		...
		},
		{
			...
			wantFilepath: filepath.Join("nexus.shitcloud.se/repository/ollama-COMBINED", "namespace", "model", "latest"),
		},
		{
			in: "model",
			want: Name{
			...
			wantFilepath: filepath.Join("nexus.shitcloud.se/repository/ollama-COMBINED", "library", "model", "latest"),
		},
		{
			in: "h/nn/mm:t",
			want: Name{
...
func TestNameparseNameDefault(t *testing.T) {
	...
	n := ParseName(name)
	got := n.String()
	want := "nexus.shitcloud.se/repository/ollama-COMBINED/library/xx:latest"
	if got != want {
...
func TestDisplayShortest(t *testing.T) {
	cases := map[string]string{
		"nexus.shitcloud.se/repository/ollama-COMBINED/library/model:latest": "model:latest",
		"nexus.shitcloud.se/repository/ollama-COMBINED/library/model:tag":    "model:tag",
		"nexus.shitcloud.se/repository/ollama-COMBINED/namespace/model:tag":  "namespace/model:tag",
